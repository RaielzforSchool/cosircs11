...
// Function to toggle the visibility of the instruction popup
...
// Function to handle the close action of the pop-up
function closePopupPermanently() {
  const popup = document.getElementById("instruction-popup");
  popup.remove(); // Completely remove the pop-up from the DOM
  const helpButton = document.querySelector(".help-btn-container");
  if (helpButton) {
      helpButton.remove(); // Remove the help button from the interface
  }
}
