class LinearEquation {
  ...
  isFinalSolution(){
    if(this.equation === `x = ${this.num2}` || this.equation == `undefinedx = ${this.num2}`){
      return true
    }else {
      return false
    }
...
count = 0;
while (equation.isFinalSolution === False){
  if(count===0){
    equation.createRandEquation();
  }
  console.log('Created Random Equation')
  possops = equation.returnPossOpList();
  possnums = equation.returnPossNumList();
  value = prompt(`Choose: ${possnums} \n`);
  operation = prompt(`Choose: ${possops} \n`)

  switch (operation){
    case '+': equtaion.performAddition(value);
    break;
    case '-': equation.performSubtraction(value);
    break;
    case '/': equation.perfromDivision(value);
  }
